<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="regionList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="序号" type="index" align="center" prop="id" width="90" />
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary"  @click="watchMessage(scope.row)" v-hasPermi="['manage:region:list']">查看详情</el-button>
    ...
    <!--查看详情对话框-->
    <el-dialog title="查看详情" v-model="watchOpen" width="600px" append-to-body>
      <el-form ref="regionRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="区域名称" prop="regionName">
...
function handleSelectionChange(selection) {
  ...
  regionNames.value=selection.map(item => item.regionName);
...
function watchMessage(row){
    ...
    console.log(JSON.stringify(loadAllParams, null, 2)); 
...
function handleDelete(row) {
  ...
  const _regionNames=row.regionName || regionNames.value;
  proxy.$modal.confirm('是否确认删除名称为"' + _regionNames + '"的区域？').then(function() {
