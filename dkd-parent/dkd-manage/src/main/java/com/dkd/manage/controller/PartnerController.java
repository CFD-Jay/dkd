...
public class PartnerController extends BaseController
{
    ...
    public TableDataInfo list(Partner partner)
    {
        ...
        startPage();
        List<PartnerVo> partnerVos = partnerService.selectPartnerVoList(partner);
     //   System.out.println(partnerVos.size());
    ...
    @PreAuthorize("@ss.hasPermi('manage:partner:resetPassword')")
    @Log(title = "合作商管理", businessType = BusinessType.ResetPassword)
    @PutMapping("/resetPwd/{id}")
    public AjaxResult resetPassword(@PathVariable Integer id) {
        String password = SecurityUtils.encryptPassword("123456");
        return toAjax(partnerService.resetPassword(password,id));
    }
}
