...
public class NodeController extends BaseController
{
    ...
    public TableDataInfo list(Node node)
    {
        ...
        List<NodeVo> NodeVos = nodeService.selectNodeVoList(node);
        return getDataTable(NodeVos);
    }
     ...
     */
    @PreAuthorize("@ss.hasPermi('manage:node:list')")
    @GetMapping("/nodeListByRegionId/{regionId}")
    public TableDataInfo nodeListByRegionId(@PathVariable("regionId") Long regionId)
    {
      //  System.out.println(regionId);
        startPage();
        //List<Node> list = nodeService.selectNodeList(node);
        List<Node> Nodes = nodeService.nodeListByRegionId(regionId);
